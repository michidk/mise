...
var title = document.querySelector("[data-admin-title]");
if (root && content && title) {
  ...
  observer.subscribe((result) => {
    ...
    document.title = `${result.data.title} \xB7 mise admin`;
  ...
  document.addEventListener("click", (event) => {
    const target = event.target instanceof Element ? event.target.closest("a[data-admin-nav]") : null;
