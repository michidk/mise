...
async function joinRoom(id) {
    ...
    try {
        ...
        viewerControl.on('close', () => {
            if (!peer?.destroyed)
                endViewer('The room is no longer available.');
        });
...
function acceptViewer(connection) {
    if (connection.metadata?.role !== 'viewer')
...
function handleRoomMessage(value) {
    ...
    switch (message?.type) {
        ...
        case 'participant-joined':
            if (isLocalPresenter() && message.peerId)
                callParticipant(message.peerId);
            break;
...
function handleViewerData(viewerId, value) {
    ...
    const now = Date.now();
    if (!viewer || !text || now - viewer.lastMessageAt < 300)
        return;
...
async function startRoomPresentation() {
    if (activePresenterId || shareRequestPending || localStream)
        return;
...
function setPresenterState(presenterId, presenterName = '') {
    if (incomingCall && incomingCall.peer !== presenterId)
        closeIncomingCall();
...
function updateRoomUI() {
    if (isRoomCreator) {
        ...
        if (isLocalPresenter())
            title.textContent = 'You’re sharing your screen';
        else if (activePresenterId)
            title.textContent = `${activePresenterName || 'A guest'} is sharing`;
        else
            title.textContent = 'The room is open';
    ...
    if (isLocalPresenter()) {
        ...
        if (activePresenterId)
            setViewerConnectionStatus('live', `${activePresenterName || 'A participant'} is sharing`);
    }
    if (!activePresenterId) {
...
function acceptIncomingScreen(call) {
    ...
    call.on('stream', (stream) => {
        if (incomingCall !== call || call.peer !== activePresenterId)
            return;
...
function callParticipant(participantId) {
    if (!peer || !participantId || participantId === peer.id || !localStream?.active || outgoingCalls.has(participantId))
        return;
    ...
    call.on('close', () => {
        if (outgoingCalls.get(participantId) !== call)
            return;
...
function appendChatMessage(message) {
    ...
    if (!container || container.querySelector(`[data-message-id="${CSS.escape(message.id)}"]`))
        return;
...
async function applyAdvancedSettings() {
    ...
    const contentHint = requestedContentHint === 'detail' || requestedContentHint === 'motion'
...
document.querySelectorAll('[data-quality]').forEach((button) => {
    button.addEventListener('click', () => {
        ...
        if (quality && quality in qualityPresets)
            setQuality(quality);
    });
});
...
document.addEventListener('click', (event) => {
    ...
    if (!target?.closest('[data-quality-trigger]') && !target?.closest('#quality-menu'))
        closeQualityMenu();
});
$('#join-form').addEventListener('submit', (event) => {
    ...
    if (!id)
        return showToast('Enter a valid room code.', 'error');
...
if (routeMatch)
    joinRoom(routeMatch[1].toLowerCase());
else
    setScreen('landing');
export {};
